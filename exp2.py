...
def define_model(source_vocab_size, target_vocab_size, src_timesteps, tar_timesteps, src_embedding_matrix, src_embedding_dim=50, n_units=200):
    ...
    model.add(Embedding(source_vocab_size, src_embedding_dim, weights=[src_embedding_matrix], input_length=src_timesteps, trainable=True))
...
print(model.summary())
plot_model(model, to_file='model_secondary.png', show_shapes=True)
# fit model
filename = 'model_secondary.h5'
...
history = model.fit_generator(
	...
	max_queue_size=2,
	callbacks=[checkpoint, tensorboard], verbose=2)
